...
def train_step(i, prng_key, d_state, g_state, d_opt, g_opt, d_apply, g_apply, real_ims, batch_size):
    ...
    d_loss_value, d_grads = value_and_grad(d_loss)(d_params, g_params, d_apply, g_apply, prng1, batch_size, real_ims)
    d_state = d_opt['update'](i, d_grads, d_state)

    g_loss_value, g_grads = value_and_grad(g_loss)(g_params, d_params, g_apply, d_apply, prng2, batch_size)
...
def train(batch_size, num_iter, digit):
    ...
    prng_key = jax.random.PRNGKey(random_seed_default)
